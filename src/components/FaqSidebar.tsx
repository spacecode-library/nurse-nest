...
import { X, Search, ChevronDown, ChevronUp, ArrowUp } from 'lucide-react';
...
export default function FaqSidebar({ isOpen, onClose }: FaqSidebarProps) {
  ...
  return (
    <>
      ...
      <div 
        className={cn(
          "fixed top-0 right-0 h-full w-full md:w-[500px] lg:w-[550px] z-50 transition-transform duration-300 ease-in-out",
          isOpen ? "translate-x-0" : "translate-x-full"
        )}
      >
        <div className="h-full bg-gradient-to-br from-[#f0f9ff] to-[#e2e8f0] backdrop-blur-md border-l border-white/20 shadow-2xl flex flex-col">
          {/* Header */}
          <div className="flex-shrink-0 p-6 border-b border-white/20 bg-white/30 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-4">
              <button
                onClick={onClose}
                className="p-2 rounded-lg hover:bg-white/20 transition-colors text-[#1e293b]"
          ...
          >
            {filteredCategories.length === 0 ? (
              ...
              <>
                {filteredCategories.map((category, categoryIndex) => (
                  <div key={category.id} className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-white/40 backdrop-blur-sm rounded-lg border border-white/30 shadow-sm">
                      ...
                      <h3 className="font-semibold text-lg text-[#1e293b]">{category.title}</h3>
                    </div>
                    ...
                    <Accordion type="single" collapsible className="space-y-2">
                      {category.faqs.map((faq, faqIndex) => (
                        ...
                        >
                          <AccordionTrigger className="px-4 py-4 hover:no-underline hover:bg-white/40 transition-colors text-left">
                            <div className="flex items-start gap-3 w-full">
                              <span className="text-lg flex-shrink-0 mt-0.5">{category.icon}</span>
                              <span className="text-base font-medium text-[#1e293b] leading-relaxed">{faq.question}</span>
                            </div>
                          </AccordionTrigger>
                          <AccordionContent className="px-4 pb-4 pt-1">
                            <div className="pl-9 space-y-4">
                              ...
                              <button 
                                onClick={scrollToTop}
                                className="flex items-center text-[#3b82f6] text-sm font-medium hover:text-[#2563eb] transition-colors"
                              >
                ...
                {/* Contact Information */}
                <div className="mt-8 p-6 bg-white/40 backdrop-blur-sm rounded-lg border border-white/30 shadow-sm">
